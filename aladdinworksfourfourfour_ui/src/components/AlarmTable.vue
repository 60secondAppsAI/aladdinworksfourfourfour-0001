        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalAlarms = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalAlarms">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Alarm</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="AlarmId" type="text" placeholder="Enter AlarmId" v-model="alarmToAdd.alarmId"></base-input>
  <base-input label="Description" type="text" placeholder="Enter Description" v-model="alarmToAdd.description"></base-input>
  <base-input label="Resolved" type="text" placeholder="Enter Resolved" v-model="alarmToAdd.resolved"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="alarms" :row-key="record => record.AlarmId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <AlarmPictureView :alarms="alarms" />
          </div>
...
import AlarmPictureView from './AlarmPictureView.vue';
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'resolved',
          visible: true,
          scopedSlots: { customRender: 'resolved' },
          sorter: true
          //sorter: (a, b) => a.resolved - b.resolved,
          //sorter: (a, b) => a.resolved.localeCompare(b.resolved),
        },
      ],
      pagination: {
        current: 1,
      ...
      sortBy: 'alarmId',           // Column to sort by
  ...
  methods: {
    ...
    async renderAlarmsTable() {
      ...
      for (let i = 0; i < this.alarms.length; i++) {
        alarmsTableData.push({
          id: i,
          alarmId: this.alarms[i].alarmId,
          year: this.alarms[i].year,
          date: this.alarms[i].date,
          competitionId: this.alarms[i].competitionId,
          alarmId: this.alarms[i].alarmId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-alarms',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForAlarmsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalAlarms = false;

      const currentDate = new Date().getTime();
      this.alarmToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.alarmToAdd);
      console.log(jsonData);
      
      const res = await AlarmService.addAlarm(jsonData);
