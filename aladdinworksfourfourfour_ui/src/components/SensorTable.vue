        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalSensors = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalSensors">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Sensor</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="SensorId" type="text" placeholder="Enter SensorId" v-model="sensorToAdd.sensorId"></base-input>
  <base-input label="Type" type="text" placeholder="Enter Type" v-model="sensorToAdd.type"></base-input>
  <base-input label="Location" type="text" placeholder="Enter Location" v-model="sensorToAdd.location"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="sensors" :row-key="record => record.SensorId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <SensorPictureView :sensors="sensors" />
          </div>
...
import SensorPictureView from './SensorPictureView.vue';
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'sensorId',
          visible: true,
          scopedSlots: { customRender: 'sensorId' },
          sorter: true
      ...
      sortBy: 'sensorId',           // Column to sort by
  ...
  methods: {
    ...
    async renderSensorsTable() {
      ...
      let sensorsTableData = [];
      for (let i = 0; i < this.sensors.length; i++) {
        sensorsTableData.push({
          id: i,
          sensorId: this.sensors[i].sensorId,
          year: this.sensors[i].year,
          date: this.sensors[i].date,
          competitionId: this.sensors[i].competitionId,
          sensorId: this.sensors[i].sensorId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-sensors',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForSensorsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalSensors = false;

      const currentDate = new Date().getTime();
      this.sensorToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.sensorToAdd);
      console.log(jsonData);
      
      const res = await SensorService.addSensor(jsonData);
