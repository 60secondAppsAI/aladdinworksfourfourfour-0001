        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalIssues = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalIssues">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Issue</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="IssueId" type="text" placeholder="Enter IssueId" v-model="issueToAdd.issueId"></base-input>
  <base-input label="Description" type="text" placeholder="Enter Description" v-model="issueToAdd.description"></base-input>
  <base-input label="DateReported" type="text" placeholder="Enter DateReported" v-model="issueToAdd.dateReported"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="issues" :row-key="record => record.IssueId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <IssuePictureView :issues="issues" />
          </div>
...
import IssuePictureView from './IssuePictureView.vue';
...
export default {
  ...
  data() {
    return {
      ...
      sortBy: 'issueId',           // Column to sort by
  ...
  methods: {
    ...
    async renderIssuesTable() {
      ...
      for (let i = 0; i < this.issues.length; i++) {
        issuesTableData.push({
          id: i,
          issueId: this.issues[i].issueId,
          year: this.issues[i].year,
          date: this.issues[i].date,
          competitionId: this.issues[i].competitionId,
          issueId: this.issues[i].issueId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-issues',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForIssuesChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalIssues = false;

      const currentDate = new Date().getTime();
      this.issueToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.issueToAdd);
      console.log(jsonData);
      
      const res = await IssueService.addIssue(jsonData);
